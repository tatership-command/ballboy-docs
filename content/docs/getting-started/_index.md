---
title: "Getting Started"
summary: "Add Ball Boy, grant permissions, and create your first league and season."
weight: 10
bookFlatSection: true
---

# Getting Started

New to Ball Boy? Start here. These pages take a server owner from "I just added the
bot" to "I have a league, a season, and a claimed team," in reading order:

1. {{< relref "/docs/getting-started/adding-ball-boy" >}} — invite the bot and run
   its guided setup flow.
2. {{< relref "/docs/getting-started/permissions-setup" >}} — check and fix Discord
   permissions with `/admin roles setup`.
3. {{< relref "/docs/getting-started/first-league" >}} — create a league, create a
   season, and claim a team.
